...
plot_grid(focus.area.plot,  hjust = -0.1, label_size = 13)
```
...
```{r network layers , fig.cap = "The figure presents the hexagonal structure. Each triangle is a tower location with some randomness implemented."}
...
```{r animation}
knitr::include_graphics("Vysoká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/working objects/antenna animation.gif")
```
...
coverage.areas %>%  
  ...
  kable_minimal() %>%
  footnote(general = "Note: `area.in.sq.km` reports the actual area that was used for the generation of the coverage. Therefore, the first layer spans over the whole focus area, the second layer over the suburban and urban area, and the third layer over the urban area.")
```
...
```{r coverage intensity, fig.cap= "The distribution is right skewed with a mean of `r round(mean(coverage.intensity$count), 2)`, a minimum of `r min(coverage.intensity$count)` and a maximum of `r max(coverage.intensity$count)` antenna(s) per tile."}
...
C.vec.df <- readRDS("Vysoká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/working objects/C.vec.df.final.rds")
# ECCDF of population distribution 
...
```{r c.vector special cases, fig.cap = "These antennas have more than 10,000 mobile devices associated to themselves. They all belong to layer 1 and are located close or within urban centers."}
...
Currently two specific estimation strategies are implemented:

-   

    (1) MLE Poisson, 100 iterations performed, non informative constant prior, equal probability P.matrix

-   

    (2) MLE Poisson, 100 iterations performed, non informative constant prior, true P.matrix

```{r estimations 1}
sample.eval <- list.files("Vysoká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/sample evaluation/", 
                            pattern = "*.rds", full.names = TRUE) %>% 
  map(readRDS)

plot_grid(sample.eval[[1]], sample.eval[[2]], 
...
```{r estimations 2}
plot_grid(sample.eval[[4]], sample.eval[[5]], 
          labels = "",  hjust = -0.1, label_size = 12,
          ncol = 2, nrow = 1)

sample.eval[[6]] %>% 
  kbl(caption = "True P.matrix, constant prior: RMSE") %>% 
  kable_minimal()
```
...
!!! Also under construction table for selected iterations will be added for next meeting

-   MLE Poisson: $D_{avg}(u,v) = \frac{1}{U}\sum_{j = 1}^{J} |u_{j} - v_{j}|$
