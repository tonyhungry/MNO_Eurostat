...
```
Mobile phones are an essential part of modern life. From going on Twitter to complain about politics to texting with friends about the latest and hottest gossip and live streaming on Facebook about an underground party to sharing immediate information in the front of the battle lines. Mobile phones have revolutionized the way we communicate with each other and how we lead our daily lives. 

Mobile phone data  EUROSTAT Coding Lab contributes to a fundamental topic of official statistics: the estimation of present population through mobile phones data (MNO). While the resident population takes into consideration only the individuals who permanently reside in a certain geographical area, the present population or **de facto population** "is composed by all individuals who are physically present in the geographic area of interest at a selected reference time" [1].

The concept of the **de facto population** allows us to better highlight the dynamic of change (e.g. movement) in the population in different periods. This is the reason why not only official statistics but also the general population and private companies are interested in this new hot topic. 

The research goal of this Coding Lab is to evaluate and compare different cutting edge estimation techniques of present population. 

Thanks to this project we had the opportunity to dig deeper into the world of spatial analysis as well as simulation studies in R. Throughout this notebook, we faced multiple challenges; the main difficulty for us was approaching to geo-spatial analysis managing a huge amount of data. This made coding even more tough. Moreover, the interpretation of the numerical results and charts was very complex. 

The notebook at relies on multiple scripts that follow a methodological chain. They are conceptualized and written in a modular fashion, making further use of the code for different parameters very easy.

In the *Generating a semi-synthetic population based on real census data* section we have retrieved population census data from the German Federal Statistical Office [2] (https://www.destatis.de/EN/Home/_node.html). This data entails population count categories on the 100m\*100m tile level. For computational reasons, this version will only focus on a subset of the tiles located in the state of Bavaria, which is situated in the south-east of Germany. We chose this area because it comprises both urban, suburban, and rural areas.

Important features, definition and  parameters we used:
- DEFINITIONS OF EVEYTHING  [ TILES, TOWER, ANTENNAS, LAYERS, OVERLAPPING/NON OVERLAPPING] -->TO DO 
...
The tiles are classified based on the number of population per tile as well its proximity to neighboring tiles in order to identify groups of tiles into different categories of urbanization.
...
# Plot read in
VT.plot <- readRDS("soká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/working objects/VT.plot.rds")
VA.plot <- readRDS("soká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/working objects/VA.plot.rds")
...
VT.example.plot <- readRDS("Vysoká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/working objects/VT.example.plot.rds")
...
We will now explain our workflow for the Simple Bayes estimate.

## (Simple Bayes)

```{r}
# We are unsure if we should implement this as an estimator because we are using it to our understanding not to its full potential: We are not defining an informative prior. Therefore we thought of mentioning it and saying that it is the base of the MLE estimators however we did not focus on the generation of good prior.
```


Simple Bayes $\hat{u}_{j} = a_{j}\sum_{i = 1}^{I} c_{i} \frac{p_{ij}}{\sum_{k = 1}^{J} p_{ik} a_{k}}$`
...
plot_grid(true.geo.eval.plot, VT.geo.eval.plot,
          labels = "Comparing Voronoi tower estimate", label_size = 12,
          align = "hv", hjust = -0.1)
...
plot_grid(true.geo.eval.plot, VA.geo.eval.plot,
          labels = "Comparing Voronoi antenna estimate", label_size = 12,
          align = "hv", hjust = -0.1)
```
...
knitr::include_graphics("Vysoká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/Estimates/Plot.files/MLE.equal.estimate_map.gif")
...
knitr::include_graphics("Vysoká škola ekonomická v Praze/Tony Wei Tse Hung - YAY/Estimates/Plot.files/MLE.oracle.estimate_map.gif")
```

```{r}
# @all: Should we implement here the SB estimator as well? We figured it is maybe not a valid evaluation because the SB estimator assumes an informative prior and this is not the case here
```
