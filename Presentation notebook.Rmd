...
knit: (function(input_file, encoding) { out_dir <- 'docs'; rmarkdown::render(input_file,
  encoding=encoding, output_file=file.path(dirname(input_file), out_dir, 'indexnew.html'))})
---
...
Currently, we have only implemented the first one.
...
```{r voronoi tower plot, fig.cap="The tower centroids were used as seeds for the creation of the Voronoi regions."}
...
tower.aggr <- coverage.areas.comp %>% 
  mutate(X.tow = case_when(tower.ID == "RT52" ~ 4493314,
                           tower.ID == "ST116" ~ 4499810,
                           TRUE ~ X.tow),
         Y.tow = case_when(tower.ID == "RT52" ~ 2799819,
                           tower.ID == "ST116" ~ 2793308,
                           TRUE ~ Y.tow)) %>%
  st_drop_geometry() %>% 
  group_by(tower.ID) %>% 
...
plot_grid(VT.plot.1, labels = "Fig. 10: Voronoi tower estimation",  hjust = -0.1, label_size = 14)
```

We have now created the Voronoi regions for our focus area with tower locations as seeds. The number of tower locations (seeds) and Voronoi regions align with each other.
...
We are currently not able to create Voronoi tesselations based on antenna coverage area centroids because multiple centroids are located outside of the focus area. A workaround for this will be developed at a later stage.
