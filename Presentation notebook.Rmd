...
setwd("~/Desktop/Eurostat/working objects")
```
...
```{r message=FALSE}
census.de.100m.tile <- readRDS("Eurostat/working objects/census.tile.final.rds") 
...
germany.raw <- readRDS("Eurostat/working objects/gadm36_DEU_1_sf.rds") 
germany = germany.raw %>%  
...
prop.class <- data.frame( 
  ...
  n = summary(a), 
  prop = round(summary(a)/1828314,digits=4)*100 
) 
...
Fig.2.1 <- ggplot(prop.class, aes(x = "", y = prop, fill = class)) + 
  geom_bar(width = 1, stat = "identity", color = "white") + 
  coord_polar("y", start = 0) + 
  geom_text(aes(y = lab.ypos, label = prop), color = "black") +  
  scale_color_ptol(breaks = c("Uninhabited", "Rural", "Suburban", "Urban")) +  
  theme_void() + 
  labs(fill="Pie chart Key") +  
  ggtitle("", subtitle = "Pie chart") 
 
result.sf = readRDS("Eurostat/working objects/result.sf.rds") 
...
```{r}
layers <- readRDS("Eurostat/working objects/radio cell layers.rds") 
coverage.areas <- readRDS("Eurostat/working objects/coverage.areas.rds") 
...
```{r testing}
knitr::include_graphics("Eurostat/working objects/antenna animation.gif")
```
...
Number of towers and antennas per layer 
```{r}
layer 
