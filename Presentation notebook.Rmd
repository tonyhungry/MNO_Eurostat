...
Fig.1 <- germany %>%  
  ...
  labs(
    ...
    caption = "Source: https://gadm.org/download_country_v3.html"
  )
...
Fig.2.2 <- result.sf %>%  
  ...
  scale_color_ptol(breaks = c("Uninhabited", "Rural", "Suburban", "Urban")) +
   labs(title= "Figure 2.2: Clustering results",
    subtitle = "Distribution of the four different categories of tiles(Uninhabited, Rural, Suburban and Urban);
    obtained thought the clustering algorithm"
  ) +theme(plot.title = element_text(size=10, face="bold", hjust = 0.5),
  plot.subtitle = element_text(size=9,hjust = 0.5))
            
ECCDF.data2 <- census.de.100m.tile %>% 
  # sample_n(1000) %>%
  mutate(pop.plot = pop + 1) %>%  
  arrange(pop.plot) %>%  
  mutate(prob = 1 / n()) %>%  
...
Fig.3.3 <- ECCDF.data2 %>%   
  ...
  labs(y = "Prob(Y > x)", x = "Mobile phones",
    subtitle = "Fig 3.3:Linear ECCDF."
  ) +theme(plot.title = element_text(size=10, face="bold", hjust = 0.5),
  plot.subtitle = element_text(size=9,hjust = 0.5))

# eccdf linear
Fn <- ecdf(census.de.100m.tile$pop)
plot(Fn)

Fn = census.de.100m.tile$pop %>%
  ecdf() %>%
  plot()

Figure2 <- ggarrange(Fig.2.1.new, Fig.2.2)
annotate_figure(Figure2,
               top = text_grob("Population Classification", color = "Black", face = "bold", size = 14))
...
```{r}
Figure3 <- ggarrange(Fig.3.1, Fig.3.2, Fig.3.3)
annotate_figure(Figure3,
               top = text_grob("Fig.3: Mobile phone density per tile", color = "Black", face = "bold", size = 14))
```
...
Fig.4 <- layers[[1]] %>% 
  ...
  scale_color_identity(name = "", 
    ...
    x = NULL,
    y = NULL,
    subtitle = "In the figure there is rapresented the exagonal structure. Each triangol is a tower location with some randomness implemented"
  )+
  theme(plot.title = element_text(size=10, face="bold", hjust = 0.5),
  plot.subtitle = element_text(size=9,hjust = 0.5))
   
Figure4 <- ggarrange(Fig.4)
annotate_figure(Figure4,
               top = text_grob("Fig.4: Radio Network- Example layer 1 (Rural)", color = "Black", face = "bold", size = 14))
```
...
Fig.5 <- coverage.areas %>%  
  ...
  labs(
    x = NULL,
    y = NULL,
    subtitle ="The circles rapresents the coverage area of each tower per layer depending on the signal strenght. As the figure shows, the signal density increases with increasing population density")+
  theme(plot.title = element_text(size=10, face="bold", hjust = 0.5),
  plot.subtitle = element_text(size=9,hjust = 0.5))

Figure5 <- ggarrange(Fig.5)
annotate_figure(Figure5,
               top = text_grob("Fig.5: Coverage per area", color = "Black", face = "bold", size = 14))
```
...
Figure6 <- ggarrange(Fig.3.1 , Fig.6)
title <- expression(atop(bold("Fig.6: Full coverage corresponding to the density"), scriptstyle("The figure on the left shows the geographical distribution of the tiles classified in three layers based on clustering algorithm. On the righ side, the coverage areas per layers are rapresented and it is possible to see that the full coverage corresponds to the population density.")))
...
```{r}
# dev.to.cell.classified <- readRDS("C:/Users/ramljak/Desktop/marco/dev.to.cell.classified.final.rds") 
...
Figure8 <- ggarrange(Fig.8)
annotate_figure(Figure8,
               top = text_grob("Fig.8: ECCDF c Vector", color = "Black", face = "bold", size = 14))
```
...
# SB_u_est_EM <- function(c.vec, P.mat, a.vec, n.iter = 10) {
#   
#   # result <- vector(mode = "numeric", length = length(a.vec))
#   result <- matrix(nrow = length(a.vec), ncol = n.iter)
#   
#   for (i in 2:n.iter){
#     P.trans.mat <- t(P.mat)
#     P.correct.mat <- P.mat
...
estimations <- readRDS("C:/Users/ramljak/Desktop/marco/working objects/sampled.est.results.rds")
...
# Evaluation
The first evaluation is executed through visual analysis based on the complete focus area. The first map shows the ground truth population. The following maps show the models in the following order and specification: 
Currently three modules are implemented:
...
```{r}
Figure9a <- ggarrange(Fig.9.SB, Fig.9.equal)
title9a <- expression(atop(bold("Fig.9a: Evaluation I"), scriptstyle("To assess the fairness of our evaluation, we conduct a visual comparison between the population distribution estimated though four  strategies and the true population. The first estimate is able to capture only the main Urban agglomerate, while the second one is more sensible.")))
annotate_figure(Figure9a,
               top = text_grob(title9a))
```

```{r}
Figure9b <- ggarrange(Fig.9.true, Fig.9.oracle)
title9b <- expression(atop(bold("Fig.9b: Evaluation II"), scriptstyle("By using the True P-matrix we can see the effect of a constat and uninformative prior against an informative prior.")))
annotate_figure(Figure9b,
               top = text_grob(title9b))
```
# References
