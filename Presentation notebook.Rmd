---
title: "Estimating present population based on Mobile Network Operator data - a simulation study - final draft 04.11.2020"
...
The link to the source code for the `census.de.100.m.tile` object is [here](https://github.com/R-ramljak/MNO_Eurostat/blob/master/code/1_Read%20in%20data.R).
...
Figure 2 shows the mobile phone density per tile in the focus area. In particular, we have 4 different colors rapresenting the clusters: unhabitated, rural, suburban and urban. 

However, to have a deeper look of the mobile phone density per tile, in the figure (write the figure number of the first ECCDF) we choose to rapresent data with an empirical cumulative complementary distribution function[using a log base 10 transformation]. 

The ECCDF is a step function with jumps `1/n` at observation values, where `i` is the number of tied observations at that value. Moreover, missing values are ignored and the objective "complementary" means that we need to substruct 1 - the cumulative probability. It is commonly used with datasets that has highly skewed distribution, as we can see in our distribution has a right skewed tail. 

As the figure suggests, 50% of the data are rapresented by unhabitated tiles with no phones; then we can go thought the graph stating that 90% of the tiles contains less than 0.5 mobile phones and the 10% left are tiles both urban and rulal containing  0.5 and above mobile phones. 
...
The link to the source code for the layers.1 object as well as the coverage.areas.1 object is [here](https://github.com/R-ramljak/MNO_Eurostat/blob/master/code/2_Radio%20cell%20generation.R).
...
In the figure 6 there is plotted an histogram representing the number of antennas per tile. The distribution is right skewed however we can see some important details as the minimum antenna per tile is one and the maximum is eleven. It is definitively possible to claim that even if we are working with synthetic data , they are a realistic values. Moreover, if a tile is covered by e.g. 3 antennas that all of these antennas are sufficiently covering the tile, this means that they have a positive probability ($p_{ij}$). 
Another detail that is possible to see from the histogram is that the mean value is 2 antennas per tile and they are not normally distributed. Last remark is that we do not need to use any log transformation since the values were good enought to have a good representation of the distribution.
